...
enable_default_watchos_architectures() {
...
set explicitly. When compilation ends, libraries are created under the prebuilt folder.\n"
  ...
  display_help_options "  -x, --xcframework\t\tbuild xcframework bundles instead of framework bundles" "  -l, --lts			build lts packages to support sdk 10.0+ devices" "      --target=watchos sdk version\toverride minimum deployment target [9.0]"
  ...
  echo -e "  --enable-watchos-bzip2\t\tbuild with built-in bzip2 support [no]"
  echo -e "  --enable-watchos-zlib\t\tbuild with built-in zlib [no]"
  echo -e "  --enable-watchos-libiconv\tbuild with built-in libiconv [no]"
...
get_common_cflags() {
  ...
  x86-64 | arm64-simulator)
    echo "-fstrict-aliasing -DWATCHOS ${LTS_BUILD_FLAG}${BUILD_DATE} -isysroot ${SDK_PATH}"
    ;;
  esac
}
...
get_app_specific_cflags() {
  ...
  ffmpeg-kit)
    ...
    ;;
  sdl)
    APP_FLAGS="-DPIC -Wno-unused-function -D__WATCHOS__"
    ;;
  shine)
...
set_toolchain_paths() {
  if [ ! -f "${FFMPEG_KIT_TMPDIR}/gas-preprocessor.pl" ]; then
    DOWNLOAD_RESULT=$(download "https://github.com/libav/gas-preprocessor/raw/v20210917/gas-preprocessor.pl" "gas-preprocessor.pl" "exit")
...
initialize_prebuilt_watchos_folders() {
  if [[ -n ${FFMPEG_KIT_XCF_BUILD} ]]; then

    echo -e "DEBUG: Initializing universal directories and frameworks for xcf builds\n" 1>>"${BASEDIR}"/build.log 2>&1

    if [[ $(is_apple_architecture_variant_supported "${ARCH_VAR_WATCHOS}") -eq 1 ]]; then
      initialize_folder "${BASEDIR}/.tmp/$(get_universal_library_directory "${ARCH_VAR_WATCHOS}")"
      initialize_folder "${BASEDIR}/prebuilt/$(get_framework_directory "${ARCH_VAR_WATCHOS}")"
    fi
    if [[ $(is_apple_architecture_variant_supported "${ARCH_VAR_WATCHSIMULATOR}") -eq 1 ]]; then
      initialize_folder "${BASEDIR}/.tmp/$(get_universal_library_directory "${ARCH_VAR_WATCHSIMULATOR}")"
      initialize_folder "${BASEDIR}/prebuilt/$(get_framework_directory "${ARCH_VAR_WATCHSIMULATOR}")"
    fi
    ...
    echo -e "DEBUG: Initializing default universal directory at ${BASEDIR}/.tmp/$(get_universal_library_directory "${ARCH_VAR_WATCHOS}")\n" 1>>"${BASEDIR}"/build.log 2>&1

    # DEFAULT BUILDS GENERATE UNIVERSAL LIBRARIES AND FRAMEWORKS
    initialize_folder "${BASEDIR}/.tmp/$(get_universal_library_directory "${ARCH_VAR_WATCHOS}")"

    echo -e "DEBUG: Initializing framework directory at ${BASEDIR}/prebuilt/$(get_framework_directory "${ARCH_VAR_WATCHOS}")\n" 1>>"${BASEDIR}"/build.log 2>&1
...
#
create_universal_libraries_for_watchos_default_frameworks() {
  local ROOT_UNIVERSAL_DIRECTORY_PATH="${BASEDIR}/.tmp/$(get_universal_library_directory "${ARCH_VAR_WATCHOS}")"
...
create_watchos_default_frameworks() {
...
create_universal_libraries_for_watchos_xcframeworks() {
...
create_frameworks_for_watchos_xcframeworks() {
...
create_watchos_xcframeworks() {
  export ARCHITECTURE_VARIANT_ARRAY=("${ARCH_VAR_WATCHOS}" "${ARCH_VAR_WATCHSIMULATOR}")
