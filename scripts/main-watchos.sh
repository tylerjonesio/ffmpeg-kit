...
if [[ -z ${WATCHOS_MIN_VERSION} ]]; then
  echo -e "\n(*) WATCHOS_MIN_VERSION not defined\n"
  exit 1
fi
...
echo -e "\nBuilding ${ARCH} platform targeting watchOS SDK ${WATCHOS_MIN_VERSION}\n"
echo -e "\nINFO: Starting new build for ${ARCH} targeting watchOS SDK ${WATCHOS_MIN_VERSION} at $(date)\n" 1>>"${BASEDIR}"/build.log 2>&1
