...
export BASEDIR="$(pwd)"
export FFMPEG_KIT_BUILD_TYPE="watchos"
...
# DETECT WATCHOS SDK VERSION
export DETECTED_WATCHOS_SDK_VERSION="$(xcrun --sdk watchos --show-sdk-version 2>>${BASEDIR}/build.log)"
echo -e "\nINFO: Using SDK ${DETECTED_WATCHOS_SDK_VERSION} by Xcode provided at $(xcode-select -p)\n" 1>>"${BASEDIR}"/build.log 2>&1
echo -e "\nINFO: Build options: $*\n" 1>>"${BASEDIR}"/build.log 2>&1
...
while [ ! $# -eq 0 ]; do
  ...
  --target=*)
    ...
    export WATCHOS_MIN_VERSION=${TARGET}
    ;;
  *)
...
disable_watchos_architecture_not_supported_on_detected_sdk_version "${ARCH_ARM64_SIMULATOR}"
...
echo -e "\nBuilding ffmpeg-kit ${BUILD_TYPE_ID}shared library for watchOS\n"
echo -e -n "INFO: Building ffmpeg-kit ${BUILD_VERSION} ${BUILD_TYPE_ID}for watchOS: " 1>>"${BASEDIR}"/build.log 2>&1
echo -e "$(date)\n" 1>>"${BASEDIR}"/build.log 2>&1
...
for run_arch in {0..14}; do
  if [[ ${ENABLED_ARCHITECTURES[$run_arch]} -eq 1 ]]; then
    ...
    . "${BASEDIR}"/scripts/main-watchos.sh "${ENABLED_LIBRARIES[@]}"
...
if [[ ${NO_FRAMEWORK} -ne 1 ]]; then
  ...
  echo -e "INFO: Skipped creating watchOS frameworks.\n" 1>>"${BASEDIR}"/build.log 2>&1
fi
